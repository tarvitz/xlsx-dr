...
class TestXlsxReader(TestCase):
    ...
    def setUp(self):
        self.filename = 'xlsx/simple.xlsx'
        self.big_file_name = 'xlsx/2.xlsx'
        self.valid_data = {
            "rows": {
                "0": [u"Fields", u"Values"],
                "1": [u"Person", u"Mary"],
    ...
    def test_read_xlsx_big_file(self):
        """ tests big xlsx file with custom set range """
        xlsx_instance = XlsxReader(filename=self.big_file_name)
        data = xlsx_instance.get_data(fmt='json', sheet_range='A1:F100')
